...
import { PvpPlayer, pvpResult, PvpUser } from '../interfaces/pvp'
...
class PvpService {
    ...
    async createRoom({ socketId, CMD, userInfo, userStatus }: PostBody) {
        ...
        const inputPlayer:PvpUser = {
            [userInfo.name]: { socketId, userStatus: newUserStatus }
        }
        await redis.hSetPvp(roomName, inputPlayer)
    ...
    async getUsers(roomName: string): Promise<string> {
        console.log('getUsers');
        const getUsers = Object.entries(await redis.hGetPvpRoom(roomName));
        return `현재 인원은 ${getUsers.length}명 입니다.\n`
    }
    ...
    async startValidation(req: Request, res: Response, next: NextFunction, roomName: string) {
        ...
        if (getUsers.length === maxUsers) {
            ...
            return undefined;
        } else if (getUsers.length > maxUsers) {
            userStatus.damage = 0;
            const newUserStatus = userStatus;
            const inputPlayer:PvpUser = {
                [userInfo.name]: { socketId, userStatus: newUserStatus }
            ...
            await redis.hSetPvp(roomName, inputPlayer)
            PVP.to(socketId).emit('printBattle', { script: `${CMD}에 입장하셨습니다.`, field: 'pvpBattle', userInfo, userStatus: newUserStatus });
            return undefined;
        }
        return 'done';
    }
    ...
    async pvpStart(roomName: string) {
        ...
        for (let i = 0; i < maxUsers; i++) {
            ...
            if (user.hp === 0) {
                script += `${i+1}. Lv${user.level} ${user.name} - 사망한 유저입니다.\n`;
                continue;
            } 
            script += `${i+1}. Lv${user.level} ${user.name} - hp: ${user.hp}/${user.maxhp}, attack: ${user.attack}, defense: ${user.defense}\n`;
        }
        return script;
    }
    ...
    async leaveRoom(userStatus: UserStatus) {
        ...
        const getUsers = Object.entries(await redis.hGetPvpRoom(roomName!))
        if (getUsers.length === 0) this.destroyRoom(roomName!);
    }

    async destroyRoom(roomName: string) {
        pvpRoomList.delete(roomName)
